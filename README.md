...
| ----------- | -------------------- | ---------------------------------------------------------------------------------------------- |
| **base**    | always               | zsh (+ powerlevel10k, sheldon), tmux (+ TPM plugins), mise, Claude config + agent skills (humanizer, archify, find-skills, all of mattpocock/skills + Leonxlnx/taste-skill — need **develop** for node); `git`, `tmux` |
